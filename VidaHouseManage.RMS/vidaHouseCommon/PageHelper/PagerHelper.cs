...
namespace System.Web.Mvc
{
    public static class PagerHelper
    {
        ...
        /// <returns></returns>
        public static string Pager(this HtmlHelper helper, string id, int currentPageIndex, int pageSize, int recordCount, object htmlAttributes, string className, PageMode mode, string path, string parm,string slug="")
        {
            ...
            builder.InnerHtml = GetNormalPage(currentPageIndex, pageSize, recordCount, mode, path, parm,slug);
        ...
        /// <param name="mode">分页模式</param>
        /// <returns></returns>
        public static string Pager(this HtmlHelper helper, string id, int currentPageIndex, int pageSize, int recordCount, object htmlAttributes, string className, PageMode mode, string path , string parm = "")
        {
            //TagBuilder builder = new TagBuilder("div");
            TagBuilder builder = new TagBuilder("table");
            builder.IdAttributeDotReplacement = "_";
            ...
            builder.AddCssClass(className);
            builder.MergeAttributes(new RouteValueDictionary(htmlAttributes));
            builder.InnerHtml = GetNormalPage(currentPageIndex, pageSize, recordCount, mode, path, parm);
            return builder.ToString();
        }
        ...
        /// <param name="mode">分页模式</param>
        /// <returns></returns>
        public static string Pager(this HtmlHelper helper, string id, int currentPageIndex, int pageSize, int recordCount, string className, PageMode mode, string path ,string parm="")
        {
            return Pager(helper, id, currentPageIndex, pageSize, recordCount, null,className, mode, path,parm);
        }
        ...
        /// <param name="mode">分页模式</param>
        /// <returns></returns>
        public static string Pager(this HtmlHelper helper, string id, int currentPageIndex, int pageSize, int recordCount, string className, PageMode mode, string path, string parm,string slug="")
        {
            return Pager(helper, id, currentPageIndex, pageSize, recordCount, null, className, mode, path, parm,slug);
        }
        /// <summary>
        /// 获取普通分页
        /// </summary>
        ...
        /// <param name="recordCount"></param>
        /// <returns></returns>
        private static string GetNormalPage(int currentPageIndex, int pageSize, int recordCount, PageMode mode, string path,string parm="")
        {
                
            int pageCount = (recordCount % pageSize == 0 ? recordCount / pageSize : recordCount / pageSize + 1);
            StringBuilder url = new StringBuilder();
            if (!string.IsNullOrEmpty(path)&&!string.IsNullOrEmpty(parm))
            {
                url.Append(path + "?pageindex={0}"+ "&strView={1}");
            }
            else
            {
                url.Append(HttpContext.Current.Request.Url.AbsolutePath + "?pageindex={0}");
           ...
           // sb.AppendFormat("总共{0}条记录,共{1}页,当前第{2}页&nbsp;&nbsp;", recordCount, pageCount, currentPageIndex);
            ...
            {
                string url1 = string.Format(url.ToString(), 1,parm);
                sb.AppendFormat("<a href={0}>首页</a>", (url1));
            }
            if (currentPageIndex > 1)
            {
                string url1 = string.Format(url.ToString(), currentPageIndex - 1,parm);
                sb.AppendFormat("<a href={0}>上一页</a>", (url1));
            }
            else
                sb.Append("<a>上一页</a>");
            if (mode == PageMode.Numeric)
                sb.Append(GetNumericPage(currentPageIndex, pageSize, recordCount, pageCount, (url.ToString()),parm));
            if (currentPageIndex < pageCount)
            {
                string url1 = string.Format(url.ToString(), currentPageIndex + 1,parm);
            ...
            {
                string url1 = string.Format(url.ToString(), pageCount,parm);
        ...
        /// <param name="recordCount"></param>
        /// <returns></returns>
        private static string GetNormalPage(int currentPageIndex, int pageSize, int recordCount, PageMode mode, string path, string parm , string slug = "")
        {

            int pageCount = (recordCount % pageSize == 0 ? recordCount / pageSize : recordCount / pageSize + 1);
            StringBuilder url = new StringBuilder();
            if (!string.IsNullOrEmpty(path) && !string.IsNullOrEmpty(parm) && string.IsNullOrEmpty(slug))
            {
                url.Append(path + "?pageindex={0}" + "&strView={1}" + "&strSlug={2}");
            }
            else
            {
                url.Append(HttpContext.Current.Request.Url.AbsolutePath + "?pageindex={0}");
            ...
            {
                string url1 = string.Format(url.ToString(), 1, parm,slug);
                sb.AppendFormat("<span><a href={0}>首页</a></span>&nbsp;", (url1));
            }
            if (currentPageIndex > 1)
            {
                ...
                sb.Append("<span>上一页</span>&nbsp;");
            if (mode == PageMode.Numeric)
                sb.Append(GetNumericPage(currentPageIndex, pageSize, recordCount, pageCount, (url.ToString()), parm,slug));
            if (currentPageIndex < pageCount)
            {
                string url1 = string.Format(url.ToString(), currentPageIndex + 1, parm);
                sb.AppendFormat("<span><a href={0}>下一页</a></span>&nbsp;", (url1));
            ...
            {
                string url1 = string.Format(url.ToString(), pageCount, parm,slug);
                sb.AppendFormat("<span><a href={0}>末页</a></span>&nbsp;", (url1));
            }
            return sb.ToString();
        }
        ...
        /// <returns></returns>
        private static string GetNumericPage(int currentPageIndex, int pageSize, int recordCount, int pageCount, string url, string parm = "",string slug="")
        {
            ...
            for (int i = k * 5 + 1; i <= k * 5 + m; i++)
            {
                if (i == currentPageIndex)
                    sb.AppendFormat("<a class='newPager'>{0}</a>", i);
                else
                {
                    string url1 = string.Format(url.ToString(), i,parm,slug);
